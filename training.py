...
def visualize_augmented_images(support_images_online, support_images_target, query_images, 
                              iteration, save_dir='visualizations', visualize_every=1000):
    """
    Visualize augmented support and query images
    Args:
        support_images_online: Support images for online encoder [[shot.tensor for shot in way] for way in ways]
        support_images_target: Support images for target encoder [[shot.tensor for shot in way] for way in ways] 
        query_images: Query images [query.tensor for query in queries]
        iteration: Current training iteration
        save_dir: Directory to save visualizations
        visualize_every: Visualize every N iterations
    """
    ...
    def tensor_to_numpy(tensor):
      ...
      if tensor.dim() == 4:  # (B, C, H, W) - remove batch dimension
          tensor = tensor[0]  # Take first sample -> (C, H, W)

      if tensor.dim() == 3:
          if tensor.shape[0] == 1:  # Grayscale (1, H, W)
              img = tensor.squeeze(0).detach().cpu().numpy()  # -> (H, W)
          elif tensor.shape[0] == 3:  # RGB (3, H, W) 
              img = tensor.permute(1, 2, 0).detach().cpu().numpy()  # -> (H, W, 3)
          else:  # (H, W, C)
    ...
    fig.suptitle(f'Augmented Images - Iteration {iteration}', fontsize=16)
    
    try:
        # Visualize support images (online vs target)
        if support_images_online and support_images_target:
            # Get first way, first shot for visualization
            support_online = support_images_online[0][0]  # First way, first shot
            support_target = support_images_target[0][0]  # First way, first shot
            
            # Support online
            img_online = tensor_to_numpy(support_online)
            axes[0, 0].imshow(img_online, cmap='gray' if len(img_online.shape) == 2 else None)
            axes[0, 0].set_title('Support Online )(Teacher)')
            axes[0, 0].axis('off')
            
            # Support target  
            img_target = tensor_to_numpy(support_target)
            axes[0, 1].imshow(img_target, cmap='gray' if len(img_target.shape) == 2 else None)
            axes[0, 1].set_title('Support Target (Student)')
            axes[0, 1].axis('off')
            
            # Difference between online and target
            if img_online.shape == img_target.shape:
                diff = np.abs(img_online - img_target)
                axes[0, 2].imshow(diff, cmap='hot')
                axes[0, 2].set_title('Difference (|Online - Target|)')
                axes[0, 2].axis('off')
            
        # Visualize query images
        if query_images:
            query_img = tensor_to_numpy(query_images[0])  # First query
            axes[1, 0].imshow(query_img, cmap='gray' if len(query_img.shape) == 2 else None)
        ...
        for i in range(3):
            for j in range(4):
                if (i == 0 and j >= 3) or (i == 1 and j >= 1) or i == 2:
                    axes[i, j].axis('off')
    
    except Exception as e:
        print(f"Error in visualization: {e}")
        # Still save an empty plot to indicate the iteration
    ...
    # Save the plot
    plt.tight_layout()
    filename = f'augmented_images_iter_{iteration:06d}.png'
    filepath = os.path.join(save_dir, filename)
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()  # Close to free memory
    
    print(f"Saved augmentation visualization: {filepath}")
...
# _log: Sacred's logger object
    ...
    # Initialize mixed precision training with bf16
    ...
    use_dual_augs = _config.get('use_dual_augs', False)
    
    if use_dual_augs:
        # Get strategy from config
        dual_aug_strategy = _config.get('dual_aug_strategy', 'default')
        online_key, target_key = myaug.get_dual_aug_strategy(dual_aug_strategy)
    ...
    print(f"online_transforms: {online_transforms}")
    print(f"target_transforms: {target_transforms}")
    print(f"About to create dataset with dual_augs={use_dual_augs}")
    if use_dual_augs:
    ...
    trainloader = DataLoader(
        ...
        drop_last=True,
        prefetch_factor=8,  # Prefetch batches to reduce waiting
        persistent_workers=True if _config['num_workers'] > 0 else False  # Keep workers alive
    )
    ...
    for sub_epoch in range(n_sub_epoches):
        ...
        for _, sample_batched in enumerate(trainloader): # trainloader is the dataloader defined in torch.utils.data , sample_batched is the batch of data
            ...
            #                    for way in sample_batched['support_mask']] 
                        # Handle SuperpixelDataset format - check for dual augmentation mode

            if use_dual_augs and 'support_images_online' in sample_batched:
                # Dual augmentation mode - separate online and target support images
                support_images_online = [[shot.float().cuda() for shot in way] 
                                        for way in sample_batched['support_images_online']]
                support_images_target = [[shot.float().cuda() for shot in way] 
                                        for way in sample_batched['support_images_target']]
                support_fg_mask_online = [[shot['fg_mask'].float().cuda() for shot in way] 
                                         for way in sample_batched['support_mask_online']]
                support_fg_mask_target = [[shot['fg_mask'].float().cuda() for shot in way] 
                                         for way in sample_batched['support_mask_target']]
                support_bg_mask_online = [[shot['bg_mask'].float().cuda() for shot in way]
                                         for way in sample_batched['support_mask_online']]
                support_bg_mask_target = [[shot['bg_mask'].float().cuda() for shot in way]
                                         for way in sample_batched['support_mask_target']]
                
                # For backward compatibility, set variables expected by model
                support_images = support_images_online  # Will be handled differently in model
                support_fg_mask = support_fg_mask_online
                support_bg_mask = support_bg_mask_online
            else:
                # Original single augmentation mode
                support_images = [[shot.float().cuda() for shot in way] 
                                  for way in sample_batched['support_images']] 
                support_fg_mask = [[shot['fg_mask'].float().cuda() for shot in way] 
                                   for way in sample_batched['support_mask']] 
                ...
                # Set to None for single augmentation mode
                support_images_online = None
                support_images_target = None
                support_fg_mask_online = None  
                support_fg_mask_target = None
                support_bg_mask_online = None
                support_bg_mask_target = None 
            ...
            # Store current iteration's images for consistent visualization
            current_support_online = support_images_online if use_dual_augs and support_images_online is not None else None
            current_support_target = support_images_target if use_dual_augs and support_images_target is not None else None
            current_query = query_images
            ########################################################################
            if i_iter % 1000 == 1:  # Print every 1000 iterations
                    if use_dual_augs and 'support_images_online' in sample_batched:
                        print(f"Iteration {i_iter}: Using DUAL augmentation forward pass")
                    else:
                        print(f"Iteration {i_iter}: Using SINGLE augmentation forward pass")
            if use_dual_augs and 'support_images_online' in sample_batched:
                query_pred, align_loss, debug_vis, assign_mats, contrastive_loss = model(
                    support_images,
                    support_fg_mask,
                    support_bg_mask,
                    query_images,
                    isval=False, val_wsize=None,
                    supp_imgs_online=support_images_online,
                    supp_imgs_target=support_images_target,
                    fore_mask_online=support_fg_mask_online,
                    fore_mask_target=support_fg_mask_target,
                    back_mask_online=support_bg_mask_online,
                    back_mask_target=support_bg_mask_target
                )
            else:
                query_pred, align_loss, debug_vis, assign_mats, contrastive_loss = model(
                    support_images,
            ...
            optimizer.step()
            # if use_dual_augs and 'support_images_online' in sample_batched:
            #     # Pass dual augmentation parameters to model
            #     query_pred, align_loss, debug_vis, assign_mats, contrastive_loss = model(
            #         support_images,
            #         support_fg_mask,
            #         support_bg_mask,
            ...
            #     )
            # else:
            #     # Original single augmentation call
            #     query_pred, align_loss, debug_vis, assign_mats, contrastive_loss = model(
            #         support_images,
            #         support_fg_mask,
            #         support_bg_mask,
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                try:
                    ...
                    if use_dual_augs and current_support_online is not None:
                        si_tensor = current_support_online[0][0].cpu()  # Use online version to match augmentation viz
                    else:
                        si_tensor = support_images[0][0].cpu()  # Fallback for single augmentation
                   ...
                   # Use the same query image as shown in augmentation visualization
                    qi_tensor = current_query[0].cpu()  # Should be [C, H, W] or [B, C, H, W]
                ...
                plt.close(fig)
                # Show augmentation visualization for the same images used in training visualization
                if use_dual_augs and current_support_online is not None and current_support_target is not None:
                    visualize_augmented_images(
                        current_support_online, 
                        current_support_target, 
                        current_query, 
                        i_iter + 1,  # Use same iteration number
                        save_dir='augmentation_visualizations',
                        visualize_every=5000  # Always visualize when training visualization happens
                    )
