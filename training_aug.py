...
from models.grid_proto_fewshot_aug import FewShotSeg
...
import dataloaders.augutils_aug as myaug # contains data augmentation functions
...
# _log: Sacred's logger object
    ...
    # Check device availability: MPS (Mac), CUDA, or CPU
    if torch.backends.mps.is_available():
        device = torch.device('mps')
        _log.info('###### Using MPS (Metal Performance Shaders) ######')
    elif torch.cuda.is_available():
        device = torch.device(f'cuda:{_config["gpu_id"]}')
        cudnn.enabled = True
        cudnn.benchmark = True
        torch.cuda.set_device(device=_config['gpu_id'])
        _log.info(f'###### Using CUDA device: {device} ######')
    else:
    ...
    def apply_dual_augmentation_to_support(support_images, support_fg_masks, support_bg_masks):
        """
        Apply dual augmentation to support images for online-target SSL learning.
        
        Args:
            support_images: List of support images [way][shot][tensor]
            support_fg_masks: List of foreground masks [way][shot][dict with 'fg_mask']
            support_bg_masks: List of background masks [way][shot][dict with 'bg_mask']
            
        Returns:
            Tuple of (online_data, target_data) where each contains augmented images and masks
        """
        online_images, online_fg_masks, online_bg_masks = [], [], []
        target_images, target_fg_masks, target_bg_masks = [], [], []
        
        for way_idx, way_images in enumerate(support_images):
            way_online_imgs, way_online_fg, way_online_bg = [], [], []
            way_target_imgs, way_target_fg, way_target_bg = [], [], []
            
            for shot_idx, shot_img in enumerate(way_images):
                fg_mask = support_fg_masks[way_idx][shot_idx]['fg_mask']
                bg_mask = support_bg_masks[way_idx][shot_idx]['bg_mask']
                
                # Convert to numpy for augmentation
                img_tensor = shot_img.cpu()
                fg_mask_tensor = fg_mask.cpu()
                bg_mask_tensor = bg_mask.cpu()
                ...
                # Handle mask shapes
                if fg_mask_tensor.dim() == 3:  # [B, H, W]
                    fg_mask_np = fg_mask_tensor.squeeze(0).numpy()  # -> [H, W]
                else:
                    fg_mask_np = fg_mask_tensor.numpy()  # Already [H, W]
                
                # Combine image and masks for dual augmentation
                # For binary masks, we'll create a combined mask where fg=1, bg=0
                combined_mask = fg_mask_np.astype(np.float32)
                
                # Create composite array: [H, W, C + 1] where last channel is the mask
                composite = np.concatenate([img_np, combined_mask[..., None]], axis=-1)
                
                # Apply dual augmentation
                (img_online, mask_online), (img_target, mask_target) = dual_transforms(
                    composite, c_label=1, c_img=3, use_onehot=False, nclass=2
                )
                
                # Convert back to tensors with batch dimension
                img_online_tensor = torch.from_numpy(img_online.transpose(2, 0, 1)).float().unsqueeze(0)  # HWC -> CHW -> BCHW
                img_target_tensor = torch.from_numpy(img_target.transpose(2, 0, 1)).float().unsqueeze(0)  # HWC -> CHW -> BCHW
                
                # Extract binary masks
                mask_online_binary = (mask_online.squeeze(-1) > 0.5).astype(np.float32)
                mask_target_binary = (mask_target.squeeze(-1) > 0.5).astype(np.float32)
                
                fg_mask_online = torch.from_numpy(mask_online_binary).float().unsqueeze(0)  # HW -> BHW
                bg_mask_online = torch.from_numpy(1.0 - mask_online_binary).float().unsqueeze(0)  # HW -> BHW
                fg_mask_target = torch.from_numpy(mask_target_binary).float().unsqueeze(0)  # HW -> BHW
                bg_mask_target = torch.from_numpy(1.0 - mask_target_binary).float().unsqueeze(0)  # HW -> BHW
                
                way_online_imgs.append(img_online_tensor)
                way_online_fg.append({'fg_mask': fg_mask_online})
                way_online_bg.append({'bg_mask': bg_mask_online})
                
                way_target_imgs.append(img_target_tensor)
                way_target_fg.append({'fg_mask': fg_mask_target})
                way_target_bg.append({'bg_mask': bg_mask_target})
                
            online_images.append(way_online_imgs)
            online_fg_masks.append(way_online_fg)
            online_bg_masks.append(way_online_bg)
            
            target_images.append(way_target_imgs)
            target_fg_masks.append(way_target_fg)
            target_bg_masks.append(way_target_bg)
        
        return (online_images, online_fg_masks, online_bg_masks), (target_images, target_fg_masks, target_bg_masks)
    ...
    # Create dual transform for SSL online-target learning
    dual_transforms = myaug.dual_transform_for_ssl({'aug': myaug.augs[_config['which_aug']]})
    ### ================================================================================== ###
    ...
    for sub_epoch in range(n_sub_epoches):
        ...
        for _, sample_batched in enumerate(trainloader): # trainloader is the dataloader defined in torch.utils.data , sample_batched is the batch of data
            ...
            # Handle SuperpixelDataset format - but don't move to CUDA yet for augmentation
            support_images_cpu = [[shot.float() for shot in way] 
                                  for way in sample_batched['support_images']] 
            support_fg_mask_cpu = [[shot for shot in way] 
                                   for way in sample_batched['support_mask']] 
            support_bg_mask_cpu = [[shot for shot in way]
                                   for way in sample_batched['support_mask']] 

            # Apply dual augmentation to get online and target versions
            (online_images, online_fg_masks, online_bg_masks), (target_images, target_fg_masks, target_bg_masks) = \
                apply_dual_augmentation_to_support(support_images_cpu, support_fg_mask_cpu, support_bg_mask_cpu)
            
            # Move augmented data to device
            online_images = [[shot.to(device) for shot in way] for way in online_images]
            online_fg_masks = [[{'fg_mask': shot['fg_mask'].to(device)} for shot in way] for way in online_fg_masks]
            online_bg_masks = [[{'bg_mask': shot['bg_mask'].to(device)} for shot in way] for way in online_bg_masks]
            
            target_images = [[shot.to(device) for shot in way] for way in target_images]
            target_fg_masks = [[{'fg_mask': shot['fg_mask'].to(device)} for shot in way] for way in target_fg_masks]
            target_bg_masks = [[{'bg_mask': shot['bg_mask'].to(device)} for shot in way] for way in target_bg_masks]
            ...
            ########################################################################
            # Pass both online and target augmented support data to the model
            query_pred, align_loss, debug_vis, assign_mats, contrastive_loss = model(online_images,
                                                                           online_fg_masks,
                                                                           online_bg_masks,
                                                                           query_images,
                                                                           target_images,
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                try:
                    ...
                    si_tensor = online_images[0][0].cpu()  # Should be [C, H, W] or [B, C, H, W]
                    ...
                    # Support mask
                    sm = online_fg_masks[0][0]['fg_mask'].cpu().numpy()
