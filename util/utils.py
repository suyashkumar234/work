"""Util functions - M1 Mac Compatible Version
...
def compose_wt_simple(is_wce, data_name):
    """
    Weights for cross-entropy loss - Device agnostic version
    """
    if is_wce:
        if data_name in ['SABS', 'SABS_Superpix', 'C0', 'C0_Superpix', 'CHAOST2', 'CHAOST2_Superpix']:
            # Create device-agnostic tensor
            weights = torch.FloatTensor([0.05, 1.0])
            # Move to appropriate device at runtime
            if torch.cuda.is_available():
                return weights.cuda()
            elif torch.backends.mps.is_available():
                return weights.to('mps')
            else:
                return weights
        else:
