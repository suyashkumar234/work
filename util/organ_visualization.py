...
class OrganContrastiveVisualizer:
    ...
    def __init__(self, dataset_name='SABS', save_dir='visualizations', superpixel_mode=False):
        """
        ...
        Args:
            ...
            superpixel_mode: If True, treats all class_ids as generic superpixels
        """
        self.dataset_name = dataset_name
        self.save_dir = save_dir
        self.superpixel_mode = superpixel_mode
        os.makedirs(save_dir, exist_ok=True)
        
        # Dataset-specific organ mappings
        self.organ_mappings = self._get_organ_mappings()
        
        # Color schemes for different organs/superpixels
        if superpixel_mode:
            self.colors = {
                'superpixel': '#FF6B6B',     # Red for all superpixels
    ...
    def _get_organ_mappings(self):
        ...
        if self.superpixel_mode:
            # For superpixel mode, all non-background classes are just "superpixel"
            return {
                0: 'background',
                1: 'superpixel',  # All superpixels are generic, not organ-specific
                2: 'superpixel',
                3: 'superpixel',
                4: 'superpixel',
                5: 'superpixel',
        ...
        elif self.dataset_name in DATASET_INFO:
...
def create_organ_visualizer(dataset_name='SABS', save_dir='visualizations', superpixel_mode=False):
    """
    ...
    """
    return OrganContrastiveVisualizer(dataset_name=dataset_name, save_dir=save_dir, superpixel_mode=superpixel_mode) 
