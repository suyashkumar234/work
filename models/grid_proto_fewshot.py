...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz = False, 
                supp_imgs_online=None, supp_imgs_target=None, fore_mask_online=None, fore_mask_target=None,
                back_mask_online=None, back_mask_target=None):
        """
        Args:
            Args:
            supp_imgs: support images (backward compatibility)
                way x shot x [B x 3 x H x W], list of lists of tensors
            fore_mask: foreground masks for support images (backward compatibility)
                way x shot x [B x H x W], list of lists of tensors
            back_mask: background masks for support images (backward compatibility)
                way x shot x [B x H x W], list of lists of tensors
            ...
            supp_imgs_online: support images for online encoder (student)
            supp_imgs_target: support images for target encoder (teacher)  
            fore_mask_online: foreground masks for online encoder
            fore_mask_target: foreground masks for target encoder
            back_mask_online: background masks for online encoder
            back_mask_target: background masks for target encoder
        """
        ...
        use_dual_augs = (supp_imgs_online is not None and supp_imgs_target is not None)
        
        if use_dual_augs:
            # Use separate augmented inputs for teacher and student
            imgs_concat_teacher = torch.cat([torch.cat(way, dim=0) for way in supp_imgs_online]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            imgs_concat_student = torch.cat([torch.cat(way, dim=0) for way in supp_imgs_target]  
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            
            #print("Using dual augmentation: teacher (online) and student (target) receive different augmented views")
        else:
            # Original behavior - same images for both encoders
            imgs_concat_teacher = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            imgs_concat_student = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
        ...
        if use_dual_augs:
            # Use separate masks for teacher and student
            fore_mask_teacher = torch.stack([torch.stack(way, dim=0) for way in fore_mask_online], dim=0)
            fore_mask_student = torch.stack([torch.stack(way, dim=0) for way in fore_mask_target], dim=0)
            back_mask_teacher = torch.stack([torch.stack(way, dim=0) for way in back_mask_online], dim=0)
            back_mask_student = torch.stack([torch.stack(way, dim=0) for way in back_mask_target], dim=0)
        else:
            # Original behavior - same masks for both encoders
