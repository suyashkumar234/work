...
from .contrastive_aug import ContrastiveLoss
...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, target_supp_imgs=None, target_fore_mask=None, target_back_mask=None, isval=False, val_wsize=None, show_viz = False):
        """
        Args:
        ...
        # Handle dual augmentation: use target images for teacher, online images for student
        if target_supp_imgs is not None and target_fore_mask is not None and target_back_mask is not None:
            # Use target augmented images for teacher encoder
            imgs_concat_teacher = torch.cat([torch.cat(way, dim=0) for way in target_supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            # Use online augmented images for student encoder  
            imgs_concat_student = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            
            # Use corresponding masks - handle dictionary format
            fore_mask_teacher = torch.stack([torch.stack([shot['fg_mask'] for shot in way], dim=0) for way in target_fore_mask], dim=0)
            fore_mask_student = torch.stack([torch.stack([shot['fg_mask'] for shot in way], dim=0) for way in fore_mask], dim=0)
            back_mask_teacher = torch.stack([torch.stack([shot['bg_mask'] for shot in way], dim=0) for way in target_back_mask], dim=0)
            back_mask_student = torch.stack([torch.stack([shot['bg_mask'] for shot in way], dim=0) for way in back_mask], dim=0)
        else:
            # Fallback to original behavior (same images for both encoders)
            imgs_concat_teacher = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            imgs_concat_student = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                    + [torch.cat(qry_imgs, dim=0),], dim=0)
            
            fore_mask_teacher = torch.stack([torch.stack([shot['fg_mask'] for shot in way], dim=0) for way in fore_mask], dim=0)
            fore_mask_student = torch.stack([torch.stack([shot['fg_mask'] for shot in way], dim=0) for way in fore_mask], dim=0)
            back_mask_teacher = torch.stack([torch.stack([shot['bg_mask'] for shot in way], dim=0) for way in back_mask], dim=0)
            back_mask_student = torch.stack([torch.stack([shot['bg_mask'] for shot in way], dim=0) for way in back_mask], dim=0)
