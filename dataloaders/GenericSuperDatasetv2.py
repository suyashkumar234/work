...
class SuperpixelDataset(BaseDataset):
    def __init__(self, 
        ...
        Args:
            ...
            online_transforms:  augmentation function for online encoder (student)
            target_transforms:  augmentation function for target encoder (teacher)
        ...
        # print(self.base_dir)
        self.img_pids = [ re.findall(r'\d+', fid)[-1] for fid in glob.glob(self.base_dir + "/image_*.nii.gz") ]
    ...
    def __getitem__(self, index):
        ...
        if self.use_dual_transforms:
            # Apply dual transforms for online-target SSL training
            for ii in range(self.num_rep):
                # Apply online transform (aggressive augmentation)
                img_online, lb_online = self.online_transforms(comp, c_img=1, c_label=1, nclass=self.nclass, use_onehot=False)
                ...
                img_online = torch.from_numpy(np.transpose(img_online, (2, 0, 1)))
                lb_online = torch.from_numpy(lb_online.squeeze(-1))
                
                # Process target version
                if img_target.ndim == 2:
                    img_target = img_target[:,:,None]
                img_target = torch.from_numpy(np.transpose(img_target, (2, 0, 1)))
                lb_target = torch.from_numpy(lb_target.squeeze(-1))

                if img_query.ndim == 2:
                    img_query = img_query[:,:,None]
                img_query = torch.from_numpy(np.transpose(img_query, (2, 0, 1)))
                lb_query = torch.from_numpy(lb_query.squeeze(-1))

                if self.tile_z_dim:
                    img_online = img_online.repeat([self.tile_z_dim, 1, 1])
                    img_target = img_target.repeat([self.tile_z_dim, 1, 1])
                    img_query = img_query.repeat([self.tile_z_dim, 1, 1])
                    assert img_online.ndimension() == 3, f'actual dim {img_online.ndimension()}'
                    assert img_target.ndimension() == 3, f'actual dim {img_target.ndimension()}'
                ...
                # Create online sample
                sample_online = {"image": img_online,
                               "label": lb_online,
                               "mean": curr_dict["mean"],
                               "std": curr_dict["std"],
                               "is_start": is_start,
                ...
                sample_target = {"image": img_target,
                               "label": lb_target,
                               "mean": curr_dict["mean"],
                               "std": curr_dict["std"],
                               "is_start": is_start,
                ...
                # Add auxiliary attributes for three samples
                if self.aux_attrib is not None:
                    for sample in [sample_online, sample_target, sample_query]:
                ...
                pair_buffer.append(sample_online)
                pair_buffer.append(sample_target)
                pair_buffer.append(sample_query)
        else:
        ...
        if self.use_dual_transforms:
            # Organize data for dual transform mode
            support_images_online = []
            support_images_target = []
            support_mask_online = []
            support_mask_target = []
            support_class = []
            ...
            # In dual transform mode, pair_buffer contains online support, target support, and independent query
            for idx, itm in enumerate(pair_buffer):
                if idx % 3 == 0:  # Online support (aggressive augmentation)
                    support_images_online.append(itm["image"])
                    binary_masks = self.getMaskMedImg(itm["label"], 1, [1])
                    support_mask_online.append(binary_masks)
                    support_class.append(1)
                    support_params.append(itm['params'])
                elif idx % 3 == 1:  # Target support (conservative augmentation)
                    support_images_target.append(itm["image"])
                    binary_masks = self.getMaskMedImg(itm["label"], 1, [1])
                    support_mask_target.append(binary_masks)
                elif idx % 3 == 2 and len(query_images) == 0:  # Use only FIRST independent query
            ...
            return {'class_ids': [support_class],
                'support_images_online': [support_images_online],
                'support_images_target': [support_images_target], 
                'support_mask_online': [support_mask_online],
                'support_mask_target': [support_mask_target],
            ...
            # Original single transform mode
            support_images = []
            support_mask = []
            support_class = []
